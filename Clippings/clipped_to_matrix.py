        ...
        ### Get Gene Name and Ensembl ID, if it there is one
                ...
                #Initialize the entry in the degraded count dictionary.  This may be redundant with the step below...
                    if gene_id not in deg_count_dict[cell_barcode].keys():
                        deg_count_dict[cell_barcode][gene_id] = 0
...
def dict_of_TSSes(gtf):

    print('Reading GTF file:', gtf)
    df = gtfparse.read_gtf(gtf)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gtf', dest='gtf', help='GTF file for filtering TSS-overlapping reads', default=None)
